...
public class Floor {
	...
	private void createSpecialRoom(List<XY> list, int floor) {
		...
		grid[list.get(4).y()][list.get(4).x()] = new EventRoom(floor); // Create Enemy
		grid[list.get(5).y()][list.get(5).x()] = new EventRoom(floor); // Create Enemy
		grid[list.get(6).y()][list.get(6).x()] = new EventRoom(floor); // Create Enemy
